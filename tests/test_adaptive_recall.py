...
def test_weak_points_prioritized_in_random_order():
    ...
    questions = generate_first_pass_questions(subject_root, n=3)
    answers = ["Medium answer for Seed Drafting.", "Strong answer for Section A.", "Weak answer for Section B with misconceptions."]
    scores = [0.6, 0.9, 0.3]
    record_session(subject_root, questions, answers, scores)

    profile = load_progress(subject_root)
    assert len(profile.weak_points) >= 1
    assert any("Section B" in wp.topic for wp in profile.weak_points), \
    ...
    assert any("Section B" in t for t in topics_in_retest), f"Section B should be prioritized. Got: {topics_in_retest}"
